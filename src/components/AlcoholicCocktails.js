...
const AlcoholicCocktails = ({ cocktails, setCocktails }) => {
  ...
  const alcohol = async () => {
    if (searchResults) {
      ...
      const alcoholResults = [...alcoholic.data.AlcoholicCocktails];

      setSearchResults(alcoholResults);
      console.log("Alcoholic", alcoholic);

      const images = alcoholic.map((item) => item.strDrinkThumb);
